...
class PostEntity {
    ...
    factory PostEntity.fromJson(Map<String, dynamic> json) => new PostEntity(
        ...
        image: json["image"],
        likes: json["likes"],
        comment: new List<CommentEntity>.from(json["comment"].map((x) => CommentEntity.fromJson(x))),
    );
    ...
    static Post _toPost(PostEntity p) {
      return new Post(p.id, p.userId, p.body, p.image, p.likes, CommentEntity.toComments(p.comment));
    }
}
...
class CommentEntity {
    ...
    factory CommentEntity.fromJson(Map<String, dynamic> json) => new CommentEntity(
        userId: json["user_id"],
